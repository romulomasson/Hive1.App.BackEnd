...
export class Pessoa {
    ...
    @Column('character varying', {
        name: 'motivo_cadastro_negado',
        nullable: true,
        length: 255,
    })
        motivoCadastroNegado: string | null

    @Column('text', { name: 'descricao_motivo_cadastro_negado', nullable: true })
        descricaoMotivoCadastroNegado: string | null

    @Column('boolean', {
        name: 'flag_cookie_aceito',
        nullable: true,
        default: () => 'false',
    })
        flagCookieAceito: boolean | null

    @OneToMany(() => Faq, (faq) => faq.pessoa)
        faqs: Faq[]

    @OneToMany(() => PessoaConvite, (pessoaConvite) => pessoaConvite.pessoa)
        pessoaConvites: PessoaConvite[]

    @OneToMany(() => PessoaEntrega, (pessoaEntrega) => pessoaEntrega.pessoa)
        pessoaEntregas: PessoaEntrega[]

    @OneToMany(() => PessoaImagens, (pessoaImagens) => pessoaImagens.pessoa)
        pessoaImagens: PessoaImagens[]

    @OneToMany(
        () => PessoaObjetivoMissao,
        (pessoaObjetivoMissao) => pessoaObjetivoMissao.pessoa
    )
        pessoaObjetivoMissaos: PessoaObjetivoMissao[]

    @OneToMany(() => Desafio, (desafio) => desafio.pessoa)
        desafios: Desafio[]

        @OneToMany(() => Parceiro, (parceiro) => parceiro.pessoa)
            parceiros: Parceiro[]
